...
import os, json, glob, time, threading, math, sys
...
# ── EMBEDDED DUMMY PATIENT GENERATOR ───────────────────────────────
# Runs as a background thread so no separate dummy_patients.py is needed
# (required for Render / any single-process deployment)

_DUM_SEV_SCALE = {"mild":1.20,"moderate":1.80,"severe":2.50}
_DUM_CLAMP = dict(
    hr=(35,180),sbp=(60,200),dbp=(30,130),rr=(6,45),
    temp=(35.0,41.5),spo2=(70.0,100.0),wbc=(1.0,35.0),
    lac=(0.3,15.0),ecg=(-0.5,3.5))
_DUM_MAX_MINUTES = 120
_DUM_HORIZON     = HORIZON  # 30

def _dcl(v,k):
    lo,hi=_DUM_CLAMP[k]; return max(lo,min(hi,float(v)))

def _make_baseline(profile):
    seed=sum(ord(c) for c in profile["pid"])
    rng=np.random.default_rng(seed); age=int(profile.get("age",55))
    adj=(age-45)*0.12
    return dict(
...
def _compute_vitals(profile, base, minute, second, noise_offset=0):
    onset=profile.get("onset",-1); sev=profile.get("sev","none")
    ...
    if onset is not None and onset>=0 and minute>=onset:
        ...
        s=prog*_DUM_SEV_SCALE.get(sev,0.85)
    ...
    return dict(
        ...
        heart_rate_bpm    =round(_dcl(hr,  "hr"),  1),
        systolic_bp_mmhg  =round(_dcl(sbp, "sbp"), 1),
        diastolic_bp_mmhg =round(_dcl(dbp, "dbp"), 1),
        resp_rate_br_min  =round(_dcl(rr,  "rr"),  1),
        temp_c            =round(_dcl(temp,"temp"),1),
        spo2_pct          =round(_dcl(spo2,"spo2"),1),
        wbc_103_ul        =round(_dcl(wbc, "wbc"), 2),
        lactate_mmol_l    =round(_dcl(lac, "lac"), 2),
        ecg_mv            =round(_dcl(ecg, "ecg"), 3),
        sepsis_label=label,
        sepsis_onset_minute=onset if (onset is not None and onset>=0) else -1,
        severity=sev,
    )

def _avg_rows(rows, profile, minute, is_early=False, future_minute=None):
    ...
    avg.update({
        ...
        "horizon_minutes":_DUM_HORIZON if is_early else 0,
    })
    return avg

def _ensure_config():
    """Create patients_config.json with defaults if missing."""
    if not os.path.exists(CONFIG_FILE):
        os.makedirs(DATA_DIR, exist_ok=True)
        with open(CONFIG_FILE,"w",encoding="utf-8") as f:
            json.dump({"patients":_DEFAULT_PATIENTS},f,indent=2)
        print("[dummy] Created {} with {} default patients".format(
            CONFIG_FILE,len(_DEFAULT_PATIENTS)))
...
def _load_profiles():
    try:
        ...
        return _DEFAULT_PATIENTS

def _save_raw(pid, buf):
    path=os.path.join(DATA_DIR,"patient_{}_raw.json".format(pid))
...
def _save_minute(pid, avg, history):
    path=os.path.join(DATA_DIR,"patient_{}_minute.json".format(pid))
    with open(path,"w",encoding="utf-8") as f:
        json.dump({"pid":pid,"latest_minute":avg,"minute_history":list(history),
                   "updated":datetime.now().strftime("%Y-%m-%d %H:%M:%S")},f,ensure_ascii=False)

def _save_early(pid, early_avg, early_history):
    path=os.path.join(DATA_DIR,"patient_{}_early.json".format(pid))
    with open(path,"w",encoding="utf-8") as f:
        json.dump({"pid":pid,"latest_early":early_avg,"early_history":list(early_history),
                   "horizon_minutes":_DUM_HORIZON,
                   "updated":datetime.now().strftime("%Y-%m-%d %H:%M:%S")},f,ensure_ascii=False)

def dummy_generator_loop():
    """Background thread that generates live_data files every second."""
    os.makedirs(DATA_DIR, exist_ok=True)
    _ensure_config()

    raw_bufs={}; sec_acc={}; early_acc={}
    min_hist={}; early_hist={}; baselines={}
    global_sec=0

    print("[dummy] Generator thread started")
    while True:
        minute=(global_sec//_DUM_MAX_MINUTES*60+global_sec//60)%_DUM_MAX_MINUTES
        minute=(global_sec//60)%_DUM_MAX_MINUTES
        ...
        for prof in profiles:
            ...
            if pid not in raw_bufs:
                ...
                early_hist[pid]=deque(maxlen=120); baselines[pid]=_make_baseline(prof)
                print("[dummy] [+] {} {}".format(pid,prof.get("name","")))

            row=_compute_vitals(prof,baselines[pid],minute,second,noise_offset=0)
            raw_bufs[pid].append(row); sec_acc[pid].append(row)
            _save_raw(pid,raw_bufs[pid])

            future_min=(minute+_DUM_HORIZON)%_DUM_MAX_MINUTES
            early_row=_compute_vitals(prof,baselines[pid],future_min,second,noise_offset=7777)
            early_acc[pid].append(early_row)

            if second==59:
                avg=_avg_rows(sec_acc[pid],prof,minute,is_early=False)
                if avg: min_hist[pid].append(avg); _save_minute(pid,avg,min_hist[pid])
                sec_acc[pid]=[]

                early_avg=_avg_rows(early_acc[pid],prof,minute,is_early=True,future_minute=future_min)
                if early_avg: early_hist[pid].append(early_avg); _save_early(pid,early_avg,early_hist[pid])
                early_acc[pid]=[]
...
async def startup():
    ...
    print("="*65)
    load_artifacts()
    # Start embedded dummy patient data generator (no separate process needed)
    dg=threading.Thread(target=dummy_generator_loop,daemon=True); dg.start()
    print("[server] Dummy generator thread started")
